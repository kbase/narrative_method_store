...
class GenomeFeatureComparator(object):
    ...
    def compare_genome_features_async(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method compare_genome_features: argument json_rpc_context is not type dict as required.')
        return self._call('GenomeFeatureComparator.compare_genome_features_async',
                          [params], json_rpc_context)[0]

    def compare_genome_features_check(self, job_id):
        resp = self._call('GenomeFeatureComparator.compare_genome_features_check', [job_id])
        return resp[0]

    def compare_genome_features(self, params, json_rpc_context = None):
        job_id = self.compare_genome_features_async(params, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.compare_genome_features_check(job_id)
